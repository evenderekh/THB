...
class THBSiteBuilder:
    ...
    def load_data(self):
        ...
        print("Loading thb.1.3.lexicon.json...")
        with open(backend / 'thb.1.3.lexicon.json', encoding='utf-8') as f:
            lex = json.load(f)
        self.mt_lexicon  = lex.get('mt',  {})
        self.lxx_lexicon = lex.get('lxx', {})
        self.vul_lexicon = lex.get('vul', {})
        print(f"  mt: {len(self.mt_lexicon):,} entries")
        print(f"  lxx: {len(self.lxx_lexicon):,} entries")
        print(f"  vul: {len(self.vul_lexicon):,} entries")
    ...
    def build_word_span(self, word: Dict[str, Any], tradition: str) -> str:
        ...
        if tradition == 'kjv':
            ...
            return f'<span class="{word_class}" data-lemma="{html.escape(surface)}">{surface}</span>'
