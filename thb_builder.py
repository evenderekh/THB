...
class VersificationMapper:
    ...
    def __init__(self, mapping_source: Path):
        ...
        # Unified file (thb.versification.json) or legacy directory
    ...
    def _load_unified(self, path: Path):
        """Load versification from unified thb.versification.json."""
...
class THBSiteBuilder:
    def __init__(self, backend_dir: str, template_path: str,
        ...
        self.alignment: Dict[Tuple[str,int,int,str], Dict[str,List[Tuple[int,str]]]] = {}
    ...
    def load_data(self):
        ...
        for trad in ('mt', 'lxx', 'vul', 'sp', 'kjv', 'dss'):
            path = backend / f'thb.{trad}.json'
            if path.exists():
                print(f"  Loading thb.{trad}.json ({path.stat().st_size // 1024 // 1024} MB)...")
        ...
        for _trad, _attr in [('mt', 'mt_lexicon'), ('lxx', 'lxx_lexicon'), ('vul', 'vul_lexicon')]:
            _path = backend / f'thb.lexicon.{_trad}.json'
        ...
        self.mapper = VersificationMapper(backend / 'thb.versification.json')

        print("Loading Aleppo Codex page index...")
        aleppo_path = backend / 'thb.aleppo.json'
        if aleppo_path.exists():
            ...
            self.aleppo_pages = []
            print("  thb.aleppo.json not found — Aleppo links disabled")
    ...
    # Maps word-level JSON tradition keys → internal tradition keys
    _ALIGN_TRAD_MAP: Dict[str, str] = {
        'WLC': 'mt', 'LXX': 'lxx', 'VUL': 'vul',
        'SP': 'sp', 'KJV': 'kjv', 'DSS': 'dss',
    }

    def _load_alignment_data(self):
        """Load thought-unit and word-level alignment from backend/alignment/.

        Populates self.alignment:
            (book, chapter, verse, tradition) → {surface: [(tid, wid_str), ...]}

        Values are ORDERED LISTS so repeated surfaces (e.g. את in multiple thoughts)
        are consumed in sequence during rendering — each encounter pops the next entry.

        MT is built from the word-level token sequence (canonical order).
        Other traditions are built from the thought-level tradition text (in thought order).
        wid comes from the word-level JSON and may be '' when no mapping exists.
        """
        backend = Path(self.backend_dir)
        thought_dir = backend / 'alignment' / 'thought' / 'wlc'
        word_dir    = backend / 'alignment' / 'word'    / 'wlc'

        if not thought_dir.exists() or not word_dir.exists():
            return

        _PUNCT = frozenset({'׃', '·', ',', '.', ';', ':', '!', '?'})

        _BOOK_ALIASES = {"Song of Solomon": "Song of Songs"}
        files_loaded = 0
        for thought_file in sorted(thought_dir.glob('*.json')):
            word_file = word_dir / thought_file.name
            # word file is optional — MT alignment will be absent for missing verses

            stem = thought_file.stem
            dot  = stem.rfind('.')
            if dot == -1:
                continue
            book = stem[:dot].replace('_', ' ')
            book = _BOOK_ALIASES.get(book, book)
            book = book.replace(' ', '')  # match chapter_counts keys e.g. "1Samuel"
            try:
                chapter = int(stem[dot + 1:])
            except ValueError:
                continue

            try:
                thought_data = json.loads(thought_file.read_text(encoding='utf-8'))
            except Exception:
                continue
            try:
                word_data = json.loads(word_file.read_text(encoding='utf-8')) if word_file.exists() else {}
            except Exception:
                word_data = {}

            all_verse_strs = set(word_data) | set(thought_data)
            for verse_str in all_verse_strs:
                try:
                    verse = int(verse_str)
                except ValueError:
                    continue

                w_thoughts = word_data.get(verse_str, [])
                t_thoughts = thought_data.get(verse_str, [])
                if not isinstance(w_thoughts, list) or not isinstance(t_thoughts, list):
                    continue

                # ── wid lookup: {int_trad: {surface: [wid_key, ...]}} ORDERED list
                # Multiple occurrences of the same surface get distinct wid_keys in
                # appearance order so "τῶν" at position 3 and "τῶν" at position 7
                # don't collapse to the same wid.
                wid_lookup: Dict[str, Dict[str, List[str]]] = {}
                for w_obj in w_thoughts:
                    tid = w_obj.get('thought_id')
                    tokens = w_obj.get('tokens', [])
                    if not isinstance(tokens, list):
                        continue
                    for wid, token in enumerate(tokens):
                        wid_key = f'{tid}:{wid}'
                        mt_surf = (token.get('token') or '').strip()
                        if mt_surf:
                            wid_lookup.setdefault('mt', {}).setdefault(mt_surf, []).append(wid_key)
                        for json_trad, int_trad in self._ALIGN_TRAD_MAP.items():
                            if int_trad == 'mt':
                                continue
                            val = token.get(json_trad)
                            if not val:
                                continue
                            for p in (s.strip() for s in val.split() if s.strip()):
                                wid_lookup.setdefault(int_trad, {}).setdefault(p, []).append(wid_key)

                # ── MT: ordered list from word-level token sequence (every token in order)
                mt_map: Dict[str, List[Tuple[int, str]]] = {}
                for w_obj in w_thoughts:
                    tid = w_obj.get('thought_id')
                    tokens = w_obj.get('tokens', [])
                    if not isinstance(tokens, list):
                        continue
                    for wid, token in enumerate(tokens):
                        surface = (token.get('token') or '').strip()
                        if surface:
                            mt_map.setdefault(surface, []).append((tid, f'{tid}:{wid}'))
                if mt_map:
                    self.alignment[(book, chapter, verse, 'mt')] = mt_map

                # ── Non-MT: ordered list from thought-level tradition text
                # Consume wid_lookup entries in sequence so repeated surfaces
                # (e.g. "τῶν" twice) get distinct wid keys.
                for json_trad, int_trad in self._ALIGN_TRAD_MAP.items():
                    if int_trad == 'mt':
                        continue
                    trad_map: Dict[str, List[Tuple[int, str]]] = {}
                    wid_counters: Dict[str, int] = {}  # surface → next index to consume
                    for t_obj in t_thoughts:
                        tid = t_obj.get('thought_id')
                        if tid is None:
                            continue
                        text = t_obj.get('traditions', {}).get(json_trad) or ''
                        if int_trad == 'dss' and text.startswith('['):
                            text = re.sub(r'^\[[^\]]+\]\s*', '', text)
                        for surface in text.split():
                            surface = surface.strip()
                            if not surface or surface in _PUNCT:
                                continue
                            wid_list = wid_lookup.get(int_trad, {}).get(surface, [])
                            idx = wid_counters.get(surface, 0)
                            wid_key = wid_list[idx] if idx < len(wid_list) else (wid_list[-1] if wid_list else '')
                            wid_counters[surface] = idx + 1
                            trad_map.setdefault(surface, []).append((tid, wid_key))
                    if trad_map:
                        self.alignment[(book, chapter, verse, int_trad)] = trad_map

            files_loaded += 1

        if files_loaded:
            print(f"  Alignment: {files_loaded} chapter(s) loaded "
                  f"({len(self.alignment)} verse-tradition keys)")
    ...
    INLINE_PUNCT_CHARS = frozenset([
        '.', ',', ';', ':', '!', '?', '—', '–',
        '·',  # · middle dot (LXX sentence divider)
        ';',  # ; Greek question mark
        '⸃',  # ⸃ right half bracket (LXX editorial)
        '⸆',  # ⸆ raised interpolation marker (LXX editorial)
    ])

    def build_word_span(self, word: Dict[str, Any], tradition: str,
                        align_tid: Optional[int] = None,
                        align_wid: str = '') -> str:
        ...
        # Skip entirely for KJV italicized words (no MT equivalent)
        ...
        # Alignment attributes — data-tid and data-wid (resolved by caller)
        # KJV italicized words are editorial insertions with no MT equivalent —
        # they get thought-level tid for context but no word-level wid.
        _is_kjv_italic = tradition == 'kjv' and word.get('italicized')
        _align_attrs = ''
        if align_tid is not None:
            _align_attrs = f' data-tid="{align_tid}"'
            if align_wid and not _is_kjv_italic:
                _align_attrs += f' data-wid="{html.escape(align_wid)}"'
        ...
        if tradition == 'lxx':
            ...
            mai = morph_thb if morph_thb else {}
            return (
                f'<span class="{word_class}"'
        ...
        if tradition == 'kjv':
            if word.get('italicized'):
                # Editorial insertion — no MT equivalent, no lexicon/Strong's lookup
                return (
                    f'<span class="{word_class}"'
                    f' data-italicized="1"'
                    f'{_align_attrs}'
            ...
            return f'<span class="{word_class}" data-lemma="{html.escape(surface)}"{_conc_attrs}{_align_attrs}>{surface}</span>'
        ...
        if tradition == 'dss':
            ...
            _recon = word.get('reconstructed', '')
            _recon_attr = f' data-reconstructed="{_recon}"' if _recon else ''
    ...
    def build_word_sequence(self, words: List[Dict[str, Any]], tradition: str,
                            ...
                            align_map: Optional[Dict[str, List[Tuple[int, str]]]] = None) -> str:
        """
        ...
        prev_cons: str = ''
        prev_surface: str = ''
        align_counters: Dict[str, int] = {}

        # Two levels of wrapper spans:
        #   thought-group — covers all words in one thought (light highlight)
        #   wid-group     — covers words sharing the same wid (word highlight)
        # Nesting: thought-group > wid-group > word-span
        current_tid: Optional[int] = None
        thought_open: bool = False
        current_wid_key: Optional[str] = None   # _align_wid string, unique per thought+pos
        wid_open: bool = False
        last_span_idx: int = -1

        def _emit_sep(sep: str) -> None:
            if sep == 'space':
                parts.append(' ')
            elif sep == 'maqqef' and last_span_idx >= 0:
                parts[last_span_idx] = parts[last_span_idx].replace(
                    '</span>', self.MAQQEF + '</span>', 1)
        ...
        for word in words:
            ...
            # Resolve alignment
            _align_tid: Optional[int] = None
            _align_wid: str = ''
            if align_map and not word.get('is_sof_pasuq'):
                _surf_key = (word.get('surface_full') if tradition == 'dss'
                             else word.get('surface', '')).strip()
                if _surf_key:
                    _lst = align_map.get(_surf_key)
                    if _lst:
                        _idx = align_counters.get(_surf_key, 0)
                        if _idx < len(_lst):
                            _align_tid, _align_wid = _lst[_idx]
                        else:
                            _align_tid, _align_wid = _lst[-1]
                        align_counters[_surf_key] = _idx + 1

            span = self.build_word_span(word, tradition,
                                        align_tid=_align_tid, align_wid=_align_wid)
            if not span:
                continue
            ...
            if parts:
                if is_sof or is_punct_tok:
                    ...
                    sep = 'maqqef' if maqqef else 'join'
                elif attach_next:
                    sep = 'join'
            ...
            # The wid key uniquely identifies a word group within a verse;
            # _align_wid already encodes tid ("1:3"), so it's inherently scoped.
            this_wid_key = _align_wid if _align_wid else None

            # Punctuation with no alignment inherits the current thought so it
            # doesn't visually break the highlight between the words around it.
            if is_punct_tok and _align_tid is None and current_tid is not None:
                _align_tid = current_tid

            tid_changed = (_align_tid != current_tid)
            wid_changed = (this_wid_key != current_wid_key)

            if tid_changed or wid_changed:
                # Close inner (wid) wrapper first
                if wid_open:
                    parts.append('</span>')
                    wid_open = False

                if tid_changed:
                    # Close outer (thought) wrapper
                    if thought_open:
                        parts.append('</span>')
                        thought_open = False
                    # Separator between thought groups (outside both wrappers)
                    _emit_sep(sep)
                    # Open thought wrapper
                    if _align_tid is not None:
                        parts.append(f'<span class="thought-group" data-tid="{_align_tid}">')
                        thought_open = True
                    current_tid = _align_tid
                else:
                    # Same thought, different wid — separator inside thought wrapper
                    _emit_sep(sep)

                # Open wid wrapper (inside thought wrapper)
                if this_wid_key is not None:
                    parts.append(
                        f'<span class="wid-group"'
                        f' data-tid="{_align_tid}"'
                        f' data-wid="{html.escape(_align_wid)}">'
                    )
                    wid_open = True
                current_wid_key = this_wid_key
            else:
                # Same thought and wid — separator inside wid wrapper (or no wrapper)
                _emit_sep(sep)

            parts.append(span)
            last_span_idx = len(parts) - 1
    ...
    def build_combined_verse_row(self, verse_num: int, verse_data_by_tradition: Dict[str, Any],
                                 ...
                                 aleppo_url: Optional[str] = None,
                                 book: str = '', chapter: int = 0) -> str:
        ...
        for tradition in traditions:
            ...
            if verse_data:
                if tradition == 'dss':
                    # Handle DSS scrolls — render all as switchable panes
                    scrolls = verse_data.get('scrolls', [])
                    if scrolls:
                        # Build tab labels for every scroll
                        tab_labels = []
                        for sd in scrolls:
                            sname = sd.get('scroll', 'DSS')
                            seen_frags: set = set()
                            frags = []
                            ...
                            tab_labels.append(f'{sname}{frag_label}')

                        # Tab bar (single scroll = no tabs, just the label)
                        if len(scrolls) == 1:
                            verse_html += f'        <div class="qumran-source">{tab_labels[0]}</div>\n'
                        else:
                            tabs_html = ''.join(
                                f'<button class="dss-tab{" active" if i == 0 else ""}" data-scroll-idx="{i}"'
                                f' data-scroll-name="{html.escape(scrolls[i].get("scroll", ""))}">'
                                f'{html.escape(tab_labels[i])}</button>'
                                for i in range(len(scrolls))
                            )
                            verse_html += f'        <div class="dss-scroll-tabs">{tabs_html}</div>\n'

                        # One pane per scroll
                        _amap = self.alignment.get((book, chapter, verse_num, tradition))
                        for i, scroll_data in enumerate(scrolls):
                            words = scroll_data.get('words', [])
                            active_cls = ' active' if i == 0 else ''
                            verse_html += f'        <div class="dss-scroll-pane{active_cls}" data-scroll-idx="{i}">\n'
                            if words:
        ...
        for book, chapter_count in self.chapter_counts.items():
            ...
            if 'mt' not in self.thb_data or mt_book_name not in self.thb_data['mt']:
                ...
                if book == 'Song of Songs':
                    alternatives.extend(['Song', 'SongOfSongs'])
    ...
    def build_mini(self, book: str = 'Genesis', chapters: int = 3,
        ...
        # Normalize book name to match chapter_counts keys (no spaces)
        book_key = book.replace(' ', '')
        # Find the canonical key case-insensitively if needed
        if book_key not in self.chapter_counts:
            match = next((k for k in self.chapter_counts if k.lower() == book_key.lower()), None)
            if match:
                book_key = match
            else:
                print(f"Error: '{book}' not found. Valid books: {', '.join(self.chapter_counts)}")
                return
        book = book_key
...
def main():
    ...
    if args.mini:
        ...
        # Copy Genesis 1 as the site root index, fixing canonical to point to /
        gen1 = Path(output_dir) / 'genesis' / '1' / 'index.html'
        root_index = Path(output_dir) / 'index.html'
        if gen1.exists():
            content = gen1.read_text(encoding='utf-8')
            content = content.replace(
                '<link rel="canonical" href="https://hebrewbible.dev/genesis/1/">',
                '<link rel="canonical" href="https://hebrewbible.dev/">'
            )
            root_index.write_text(content, encoding='utf-8')
            print("Copied genesis/1/index.html -> index.html (canonical patched to /)")
